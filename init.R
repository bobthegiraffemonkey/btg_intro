...
letter_vertices_and_edgelist = get_letter_vertices_and_edgelist(word_split, indents)
...
undir_edge_properties = c("v1", "v2", "id", "col", "length", "segments")
...
for (i in 1:max(undir_edges[,"id"])){
  ...
  rand_colours = colours[get_colour_cycle(length(poly))]
